...
export default function TextWindow({ title, content, onClose, zIndex, onFocus, width = 400, height = 300, initialPosition = { top: 100, left: 100 }, fontSize = 21, dragAnywhere = false, useThemedScrollbar = false, boldTimes = false }) {
  ...
  const renderedContent = useMemo(() => {
    ...
    const patterns = [
      ...
      /(cssociety@depaul\.edu)/g,
      /(laptop, charger, student ID)/g,
      /(DePaul's CDM \(College of Computing and Digital Media\) building, located at 243 S Wabash Ave, Chicago, IL 60604)/g,
      /(All students are welcome!)/g,
      /(minimum of 2 people and a maximum of 6 people)/g,
      /(completely free!)/g,
